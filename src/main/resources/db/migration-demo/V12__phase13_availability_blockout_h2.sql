-- Phase 13 (H2): Convert availability from mutable inventory counters to blockout periods
-- Existing seed rows represented inventory counts; clear them before enabling blockout-only semantics.
DELETE FROM availability;

ALTER TABLE availability ADD COLUMN IF NOT EXISTS reason VARCHAR(255);
